...
a.id=h.attr("id")+"-dt-";a.scrollbar=a.scrollbar===true?"auto":"hidden";c.each("id width height scrollbar background bodyground bordercolor".split(" "),function(g,c){b=b.replace(RegExp("\\["+c+"\\]","img"),a[c])});var g=document.createElement("style");g.type="text/css";g.styleSheet?g.styleSheet.cssText=b:g.innerHTML=b;document.getElementsByTagName("head")[0].appendChild(g)},l=function(){var b=a.id;h.append('<div class="doticker-wrap"><div id="'+b+'head"></div><div id="'+b+'body"></div><div id="'+
...
a.id+'doc">'+c+"</div>")}});c.ajax({url:a.url_user,success:function(b){m(b);c.ajax({url:a.url_list,success:function(b){a.codes=[];var c=0;if(b.results!==void 0){var c=b.results.length<a.maxCodes?b.results.length:a.maxCodes,f,d;for(f=0;f<c;f++)d=b.results[f],a.codes[f]=document.createElement("div"),a.codes[f].innerHTML='<div class="'+a.id+'doc"><a href="'+d.url+'" target="_blank"><img src="'+d.thumbnail["100"]+'" title="'+d.title+'" /></a><div><h5><a href="'+d.url+'" target="_blank">'+d.title+"</a></h5><p>Last modified "+
d.modified_date.split(" ")[0]+"<br />views "+d.statistic.pageview+" / forked "+d.statistic.forked+" / &hearts; "+d.statistic.favorite+"</p></div></div>"}a.current=a.maxCodes=c;a.timer=null;a.timeout=a.interval;if(a.loop!==true)a.interval=a.duration=0;c&&i()}})}})})();return h}})(jQuery);